...
def takepic(yaw):
    ...
    dt_string = now.strftime("%d-%m-%Y_%H-%M-%S")
    yaw = round(yaw, 1)
    variable = 'Hannah_' + '{p}_{yawval}_{timeval}'.format(p=1, yawval = yaw, timeval = dt_string) + '.jpg'
    # print(variable)
...
while True:
    ...
    mag_offset = [5.8000000000000001, -12.250000000000002, 38.1500000000000006] #Insert mag_offset calibration values here
    ...
    if orbit_count<=3:
        if yaw > 140 and yaw < 160:
            takepic(yaw)
        #Capture image for poster board 2
        if yaw > 250 and yaw < 270:
            takepic(yaw)
        #Capture image for poster board 3
        if (yaw > 345 and yaw < 360) or (yaw > 0 and yaw < 20):
            takepic(yaw)
