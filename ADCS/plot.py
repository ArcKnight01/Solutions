...
def animate(i, xs, type,y1,y2,y3, mag_offset, gyro_offset, initial_angle, NORTH, weight):
    if len(y1) ==0:
    ...
    if type == 'am':
       ...
       if len(xs) == 1:
           ...
           delT = xs[-1] - xs[-2]
           y1.append(roll_gy(prev_ang[0],delT,gyroX))
           y2.append(pitch_gy(prev_ang[1],delT,gyroY))
       ...
       ax.plot(xs,y2,label = "Pitch")
       ax.plot(xs,y3,label = "Yaw")
       plt.title('Roll Pitch Yaw from North, Using Accelerometer and Magnetometer')
       plt.ylabel('deg')
    elif type == 'fusion':
       if len(xs) == 1:
           y1.append(prev_ang[0])
           ...
           y1.append(roll_F(prev_ang[0],delT,gyroX,roll_AM,weight))
           y2.append(pitch_F(prev_ang[1],delT,gyroY,pitch_AM,weight))
           y3.append(yaw_F(prev_ang[2],delT,gyroZ,yaw_AM,weight))
       ax.clear()
       ax.plot(xs,y1,label = "Roll")
       ax.plot(xs,y2,label = "Pitch")
       ax.plot(xs,y3,label = "Yaw")
       plt.title('Roll Pitch Yaw, Using Accelerometer, Magnetometer, and Gyroscope Fusion')
       plt.ylabel('deg')
    else:
...
def plot_data(type = 'am',weight = 0.9):
    mag_offset = calibrate_mag() 
    initial_angle = set_initial(mag_offset)
    NORTH = find_north()
    gyro_offset = calibrate_gyro()
    ani = animation.FuncAnimation(fig, animate, fargs =(xs,type,y1,y2,y3,mag_offset,gyro_offset,initial_angle,NORTH,weight), interval = 1000)
    plt.show()
